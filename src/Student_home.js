import React from "react";
import './Student_home.scss';
import { Navigate, useNavigate } from "react-router-dom";
export default function Student_home()
{
{/* <script src="https://kit.fontawesome.com/a7e9f794eb.js" crossorigin="anonymous"></script> */}
const navigate=useNavigate();
function handleSubmit1(){
navigate('/studentsuc');
}
function handleSubmit2(){
  window.location.href = ('https://drive.google.com/drive/folders/1h5nB1mpJkd4_kjc_fFmYNJQXZyj11Xi7?usp=sharing');
  }
return(
    <div>
<div class="split left">
  <div class="centered">
    <img src="data:image/png;base64," alt="Avatar woman"/>
    <h2 class="heading">Hi there!</h2>
    <h2 class="heading">SELECT ONE OF </h2>
    <br/>
    <p class="para">the options.</p>
  </div>
</div>

<div class="split right">
  <div class="centered">
  <main className="main">
  <button class="glow-on-hover btn1" type="button" onClick={handleSubmit1}>Rebate / Complain Filling</button>
  <button class="glow-on-hover btn1" type="button" onClick={handleSubmit2}>Mess bill / Imp. Notice</button>
        {/* <button class="btn1"><span>Rebate/Complain Filling</span></button>
        <button class="btn1"><span>Mess bill</span></button> */}
        
    </main>
</div>
</div>
</div>
)
}